...
c Note that the source and receiver are located at the bottom of the layer.
      ...
      IF (src_lay.LT.rcv_lay) THEN	!如果源与检波器层数反了，没有关系
         i = src_lay
	 src_lay = rcv_lay
	 rcv_lay = i
      ENDIF
      IF (num_lay .GT. max_num_lay) THEN	!max_num_lay=2000
      ...
      do i=1,num_lay
        read(*,*) thk(i),vps(1,i)
	if (vps(1,i).LT.Vmin) vps(1,i) = Vmin	!Vmin=0.001
        vps(1,i) = 1./vps(1,i)**2
	vps(2,i) = vps(1,i)
      enddo
...
c reflected arrivals from above.
	...
	enddo
c output
        write(*,'(f8.4,f8.5,f8.5,f8.5,f8.5)')x,t0,td,REAL(p0),REAL(pd) 	!t0是最小的
      enddo
